...
$dash_link     = 'supervisor_dashboard.php';

// 3. Search & Filter Logic (EXACTLY matched to Lecturer)
...
// 5. Main Data Query (Counts Global Evals AND Personal Evals)
$query = "SELECT 
              ...
              (SELECT COUNT(*) FROM assessments WHERE internship_id = i.internship_id AND assessment_type = 'Industry') AS my_eval_count
          FROM students s
...
<div class="dashboard-container sup-theme">

    <div class="page-header-flex">
        <div>
            <span class="stat-label sup-stat-label">Evaluation Portal</span>
            <h1 class="page-title-main sup-page-title-main">Select Intern to Evaluate</h1>
            <a href="<?php echo $dash_link; ?>" class="back-link sup-back-link">&larr; Back to Dashboard</a>
        </div>
    </div>
    ...
    <?php if (isset($_GET['msg']) && $_GET['msg'] === 'evaluated'): ?>
        <div class="edit-notice success" style="margin-bottom: 30px;">
            ...
            <div>
                Intern <strong><?php echo htmlspecialchars(urldecode($_GET['name'] ?? '')); ?></strong> has been evaluated successfully.
            </div>
        </div>
    <?php endif; ?>
    ...
    <div class="stat-card filter-form-card">
        <form method="GET" action="evaluate_list_supervisor.php" class="filter-form filter-form-compact">
            ...
            <div class="filter-form-group">
                <input type="text" name="search" class="filter-input sup-filter-focus" placeholder="Search by ID or Name..." value="<?php echo htmlspecialchars($search); ?>">
            </div>

            <div class="filter-select-group" style="flex: 1;">
                <select name="status_filter" class="search-select sup-filter-focus">
            ...
            <div class="filter-select-group" style="flex: 1;">
                <select name="sort" class="search-select sup-filter-focus">
            ...
            <div class="filter-actions">
                <button type="submit" class="btn-sup-filter">Filter Interns</button>
                <?php if ($search !== "" || $status_filter !== ""): ?>
                    <a href="evaluate_list_supervisor.php" class="sup-clear-link">Clear All</a>
                <?php endif; ?>
            </div>
                    ...
                    <tr class="table-row">
                        ...
                        <td class="table-td">
                            <?php if ($row['company_name'] && $row['company_name'] !== 'Pending Placement'): ?>
                                <a href="evaluate_supervisor.php?id=<?php echo urlencode($row['internship_id']); ?>"
                                   class="action-link sup-edit">
