...
if (isset($_POST['add_assessor'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        if (mysqli_query($conn, "INSERT INTO users (user_id, username, password, full_name, role) VALUES ('$uid', '$user', '$pass', '$name', 'Assessor')")) {
            // Encode the name and attach it to the URL
...
<div class="stat-card" style="margin-bottom: 40px; min-height: auto;">
    <h2 style="font-size: 1.25rem; color: var(--primary-dark); margin-bottom: 20px;">Register New Assessor</h2>
    <form method="POST" action="manage_assessors.php">
        ...
        <button type="submit" name="add_assessor" class="btn-primary" style="margin-top: 25px;">Add Assessor to Database</button>
    </form>
</div>
...
<div class="stat-card" style="padding: 0; overflow: hidden; min-height: auto;">
    ...
    <table style="width: 100%; border-collapse: collapse; text-align: left;">
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM users WHERE (full_name LIKE '%$search_query%' OR user_id LIKE '%$search_query%' OR username LIKE '%$search_query%') AND role IN ('Admin', 'Assessor') ORDER BY role ASC, full_name ASC";
