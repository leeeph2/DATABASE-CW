...
$stmt = $conn->prepare("
    SELECT s.student_name, s.student_id, i.internship_id, i.company_name, p.programme_name
    FROM students s
    INNER JOIN internships i ON s.student_id = i.student_id
    INNER JOIN programmes p ON s.programme_id = p.programme_id
    WHERE s.student_id = ?
    AND (i.lecturer_id = ? OR i.supervisor_id = ?)
");
$stmt->bind_param("sss", $student_id, $assessor_id, $assessor_id);
$stmt->execute();
...
$check_stmt = $conn->prepare("SELECT * FROM assessments WHERE internship_id = ? AND assessment_type = 'Academic' AND assessor_type = ? LIMIT 1");
$check_stmt->bind_param("ss", $internship_id, $assessor_type);
$check_stmt->execute();
...
$rubrics = [
    ...
    ['id' => 'score_presentation', 'label' => 'Written Report Presentation',            'w' => 15],
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($existing) {
            $upd = $conn->prepare("UPDATE assessments SET assessor_id=?, score_tasks=?, score_safety=?, score_theory=?, score_presentation=?, score_clarity=?, score_learning=?, score_project_mgmt=?, score_time_mgmt=?, total_mark=?, comments=?, date_evaluated=NOW() WHERE internship_id=? AND assessment_type='Academic' AND assessor_type=?");
            $upd->bind_param("sdddddddddsss",
                $assessor_id,
                ...
                $final_score, $feedback, $internship_id, $assessor_type
            );
            $upd->execute();
        } else {
            $new_id = "ACAD-" . strtoupper(bin2hex(random_bytes(4)));
            $ins = $conn->prepare("INSERT INTO assessments (assessment_id, internship_id, assessor_id, assessment_type, assessor_type, score_tasks, score_safety, score_theory, score_presentation, score_clarity, score_learning, score_project_mgmt, score_time_mgmt, total_mark, comments, date_evaluated) VALUES (?, ?, ?, 'Academic', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $ins->bind_param("ssssddddddddds",
                $new_id, $internship_id, $assessor_id, $assessor_type,
                $scores['score_tasks'], $scores['score_safety'], $scores['score_theory'],
                $scores['score_presentation'], $scores['score_clarity'], $scores['score_learning'],
                $scores['score_project_mgmt'], $scores['score_time_mgmt'],
        ...
        mysqli_commit($conn);
        header("Location: evaluate_list_lecturer.php?msg=evaluated&name=" . urlencode($student['student_name']));
        exit();
    } catch (Exception $e) {
...
<div id="eval-page">
    ...
    <div class="ep-header-row">
        <div>
            <a href="evaluate_list_lecturer.php" class="ep-breadcrumb">&larr; Back to Registry</a>
            <h1 class="ep-page-title">Assessment Entry Portal</h1>
        </div>
    ...
    <div class="ep-student-card">
        <div style="display: flex; gap: 14px; align-items: center; flex: 1 1 100%;">
            <div style="width: 46px; height: 46px; background: #eff6ff; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; color: var(--primary-blue); border: 1px solid #dbeafe; font-size: 1.1rem; flex-shrink: 0;">
            ...
            <div>
                ...
                <span class="ep-student-field-label" style="margin-top: 3px;"><?php echo htmlspecialchars($student['programme_name']); ?></span>
            </div>
        </div>
        <div>
            <span class="ep-student-field-label">Company Placement</span>
            <span class="ep-student-field-value"><?php echo htmlspecialchars($student['company_name'] ?? 'Pending Placement'); ?></span>
        </div>
        ...
        <div class="ep-main-grid">
            ...
            <div class="ep-right-panel">
                ...
                <div class="ep-feedback-card">
                    <span class="ep-feedback-label">Justification / Comments</span>
...
(function () {
    const rubrics = <?php
        echo json_encode(array_map(fn($r) => ['id' => $r['id'], 'w' => $r['w']], $rubrics));
    ?>;
    ...
    // Run once on load (for pre-filled re-evaluate case)
    recalculate();
})();
</script>
