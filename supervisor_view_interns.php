...
// 5. Main Data Query (FIXED)
$query = "SELECT 
            s.student_id,
            s.student_name,
            i.company_name,
            i.internship_id,
            (SELECT AVG(total_mark) FROM assessments WHERE internship_id = i.internship_id AND assessment_type = 'Industry') as avg_marks,
...
<div class="dashboard-container sup-theme">
    ...
    <div class="page-header-flex">
        <div>
            <span class="stat-label sup-stat-label">Intern Directory</span>
            <h1 class="page-title-main sup-page-title-main">View Intern Progression</h1>
            <a href="<?php echo $dash_link; ?>" class="back-link sup-back-link">&larr; Back to Dashboard</a>
        </div>
    </div>

    <div class="stat-card filter-form-card">
        <form method="GET" action="supervisor_view_interns.php" class="filter-form filter-form-compact">

            <div class="filter-form-group">
                <input type="text" name="search" placeholder="Search by ID or Name..." class="filter-input sup-filter-focus" value="<?php echo htmlspecialchars($search); ?>">
            </div>

            <div class="filter-select-group" style="flex: 1;">
                <select name="status" class="search-select sup-filter-focus">
            ...
            <div class="filter-actions">
                ...
                <?php if ($search !== "" || $filter_status !== ""): ?>
                    <a href="supervisor_view_interns.php" class="sup-clear-link">Clear All</a>
                <?php endif; ?>
            </div>
        ...
        <table class="data-table">
            <thead>
                <tr class="table-head-row">
                    ...
                    <th class="table-th">Industry Grade</th>
            ...
            <tbody>
                <?php if (mysqli_num_rows($result) > 0): ?>
                    ...
                    <tr class="table-row">
                        ...
                        <td class="table-td"><?php echo htmlspecialchars($row['company_name'] ?? '—'); ?></td>
                        <td class="table-td">
                        ...
                        <td class="table-td">
                            <?php if ($row['total_evals'] >= 2): ?>
                                <a href="view_final_report.php?id=<?php echo $row['internship_id']; ?>"
                                   class="action-link sup-edit">VIEW REPORT</a>
                            <?php else: ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="6" class="empty-state-td">No records found matching your criteria.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
