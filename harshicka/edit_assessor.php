...
// Check if the target user is an Admin before allowing edits on an 'Assessor' page
if ($assessor_data['role'] === 'Admin' && $assessor_data['user_id'] !== $_SESSION['user_id']) {
    die("Error: You cannot edit other Admin accounts from this interface.");
}
...
if (isset($_POST['update_assessor'])) {
    ...
    $role = mysqli_real_escape_string($conn, $_POST['role']); // New field
    $new_password = $_POST['password'];

    if (!empty($new_password)) {
        $hashed_pass = password_hash($new_password, PASSWORD_DEFAULT);
        $update_sql = "UPDATE users SET username='$username', full_name='$full_name', role='$role', password='$hashed_pass' WHERE user_id='$uid'";
    } else {
        $update_sql = "UPDATE users SET username='$username', full_name='$full_name', role='$role' WHERE user_id='$uid'";
    }
    ...
    if (mysqli_query($conn, $update_sql)) {
        ...
        $assessor_data['role'] = $role; // Update local role
    } else {
            ...
            <div class="form-grid">
                <div class="input-group">
                    ...
                    <select name="role" required>
                        <option value="Lecturer" <?php if($assessor_data['role'] == 'Lecturer') echo 'selected'; ?>>Lecturer</option>
                        <option value="Supervisor" <?php if($assessor_data['role'] == 'Supervisor') echo 'selected'; ?>>Supervisor</option>
                    </select>
                </div>
